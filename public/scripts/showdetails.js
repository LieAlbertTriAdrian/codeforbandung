...
var ProjectPostBox = React.createClass({
  loadProjectMembersFromServer: function() {
        var data = { projectId: projectId };
        console.log("loadProjectMembersFromServer");
        $.ajax({
  ...
  render: function() {
    var ProjectPostNodes;
    if (this.state.data.length <= 0) {
      ProjectPostNodes = "There is no member in this project.";
    }
    else {
      ProjectPostNodes = this.state.data.map(function(member) {
        return (
          <ProjectPost name={member.user_id} key={member.id} position={member.project_position} description={member.job_description} />
        );
      });
    }
    return (
...
var ProjectPostForm = React.createClass({
  getInitialState: function() {
      return { users: '', name: '1', position: '', description: '', projectId: projectId };
  },
  loadUsersFromServer: function() {
